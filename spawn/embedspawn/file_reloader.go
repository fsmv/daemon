	...
	for {
		...
		case <-sigs:
			...
			for _, refresh := range f {
				...
				if err != nil {
					log.Printf("Failed opening file for refresh %#v. You can send another SIGUSR1 to this binary to retry. Error message: %v", refresh.fileName, err)
					dataFile.Close()
					errCount++
					continue
				}
				if n, err := io.Copy(refresh.writePipe, dataFile); err != nil {
					log.Printf("Failed to refresh file on write to the OS pipe for %#v (wrote %v bytes): %v",
